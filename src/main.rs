...
fn main() {
    ...
    let (ws_stream, _) =TOKIO_RUNTIME.block_on(
    ...
    // Set ping task that sends the hashed config
    // TODO: send the actual config
    ...
    while let Some(Ok(message)) = read.next().await {
        match message {
            ...
            Message::Binary(data) => {
                ...
                let _v: config::KongConfig = serde_json::from_str(&s).unwrap();
            }
            Message::Ping(_) => {
